...
export default function DemoAgeGroupSection({ productGroup, filter }) {
  ...
  useEffect(() => {
    setLoading(true)
    setError('')
    demoApi.getDemoAgeGroup(productGroup, filter)
      .then(r => setData(r.data || []))
      .catch(() => setError('Yaş grubu verisi yüklenemedi.'))
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-white dark:bg-[#0d1f3c] border border-slate-200 dark:border-white/7 rounded-xl p-5">
        <div className="flex items-center justify-between mb-1">
          <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">Yaş Grubu Dağılımı</p>
        ...
        <ResponsiveContainer width="100%" height={230}>
          <BarChart data={sortedData} margin={{ top: 10, right: 10, left: -15, bottom: 40 }}>
            ...
            />
            <Tooltip content={<CustomTooltip metric={metric} />} cursor={{ fill: 'rgba(139,92,246,0.08)' }} />
            <Bar dataKey={metric} radius={[3, 3, 0, 0]} maxBarSize={28}>
              {sortedData.map((entry, index) => (
                <Cell 
                  key={index} 
                  fill={entry[metric] === maxValue ? '#8b5cf6' : '#c4b5fd'} 
                />
              ))}
            </Bar>
          </BarChart>
          ...
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-white dark:bg-[#0d1f3c]">
              <tr className="border-b border-slate-100 dark:border-white/8">
                <th className="text-left pb-2 font-semibold text-slate-400 uppercase tracking-wide">Yaş</th>
            ...
            <tbody>
              {sortedData.map((item, i) => {
                const isMax = item[metric] === maxValue
                return (
                  <tr key={i} className={`border-b border-slate-50 dark:border-white/4 ${isMax ? 'bg-violet-50 dark:bg-violet-500/10' : ''}`}>
                    <td className={`py-2 font-medium ${isMax ? 'text-violet-600 dark:text-violet-400' : 'text-slate-700 dark:text-slate-300'}`}>
                      {item.label}
                    </td>
                    ...
                    <td className={`py-2 text-right font-semibold ${isMax ? 'text-violet-600 dark:text-violet-400' : 'text-slate-700 dark:text-slate-200'}`}>
