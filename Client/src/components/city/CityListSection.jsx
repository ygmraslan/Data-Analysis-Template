...
export default function CityListSection({ productGroup, onCitySelect, selectedCity, filter }) {
  ...
  useEffect(() => {
    ...
    cityApi.getCityList(productGroup, filter)
      ...
      .catch(() => setError('İl listesi yüklenemedi.'))
    ...
    <div className="bg-white dark:bg-[#0d1f3c] border border-slate-200 dark:border-white/7 rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">
          İl Bazlı Üretim Sıralaması
          <span className="ml-2 text-xs font-normal text-slate-400">({data.length} il)</span>
        </p>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-slate-100 dark:border-white/8">
              <th className="text-left pb-2.5 font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wide">İl</th>
          ...
          <tbody>
            {paged.map((row, i) => {
              ...
              return (
                ...
                >
                  <td className={`py-2.5 font-bold ${isSelected ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-800 dark:text-slate-200'}`}>
                    {row.city}
                  </td>
            ...
            {Array.from({ length: Math.max(0, PAGE_SIZE - paged.length) }).map((_, i) => (
              <tr key={i} className="border-b border-slate-50 dark:border-white/4">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-100 dark:border-white/6">
          <span className="text-xs text-slate-400">
            {page * PAGE_SIZE + 1}–{Math.min((page + 1) * PAGE_SIZE, sorted.length)} / {sorted.length} il
          </span>
