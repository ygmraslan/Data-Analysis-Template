...
export default function RegionHeatmapSection({ productGroup, filter }) {
  const [data,    setData]    = useState([])
  const [loading, setLoading] = useState(true)
  const [error,   setError]   = useState('')

  const fileName = useMemo(() => {
    if (!data.length) return `NetPrim_Bolge_Heatmap_${productGroup}.xlsx`
    const first = data[0].week
    const last  = data[data.length - 1].week
    return `NetPrim_Bolge_Heatmap_${productGroup}_${first}-${last}.xlsx`
  }, [data, productGroup])

  const { trigger: handleExport, loading: exportLoading } = useExport(
    () => exportRegionHeatmap(productGroup, filter),
    fileName
  )
  ...
  useEffect(() => {
    setLoading(true)
    setError('')
   regionApi.getRegionHeatmap(productGroup, filter)
      .then(r => setData(r.data))
      .catch(() => setError('Heatmap verileri yüklenemedi.'))
      .finally(() => setLoading(false))
  }, [productGroup, filter])

  if (loading) return <RegionHeatmapSkeleton />
  if (error)   return <div className="flex items-center justify-center py-10 text-sm text-red-400">{error}</div>

  const mapped = data.map(d => ({ brand: d.region, week: d.week, avgNetPremium: d.avgNetPremium, policyRatio: d.policyRatio }))

  return (
    <div className="bg-white dark:bg-[#0d1f3c] border border-slate-200 dark:border-white/7 rounded-xl p-5">
      <HeatmapTable data={mapped} onExport={handleExport} exportLoading={exportLoading} rowLabel="Bölge" />
    </div>
  )
}