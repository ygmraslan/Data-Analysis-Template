...
export default function CompanyHeatmapSection({ productGroup, filter }) {
  const [data,    setData]    = useState([])
  const [loading, setLoading] = useState(true)
  const [error,   setError]   = useState('')

  const fileName = useMemo(() => {
    if (!data.length) return `SirketGecis_Heatmap_${productGroup}.xlsx`
    const weeks = [...new Set(data.map(d => d.week))]
    const first = weeks[0]
    const last  = weeks[weeks.length - 1]
    return `SirketGecis_Heatmap_${productGroup}_${first}-${last}.xlsx`
  }, [data, productGroup])

  const { trigger: handleExport, loading: exportLoading } = useExport(
    () => exportCompanyHeatmap(productGroup,filter),
    fileName
  )
  ...
  useEffect(() => {
    setLoading(true)
    setError('')
    companyApi.getCompanyHeatmap(productGroup, filter)
      .then(r => setData(r.data))
      .catch(() => setError('Heatmap verileri yüklenemedi.'))
      .finally(() => setLoading(false))
  }, [productGroup, filter])

  if (loading) return <CompanyHeatmapSkeleton />
  if (error)   return <div className="flex items-center justify-center py-10 text-sm text-red-400">{error}</div>

  const mapped = data.map(d => ({ brand: d.company, week: d.week, avgNetPremium: d.avgNetPremium, policyRatio: d.policyRatio }))
