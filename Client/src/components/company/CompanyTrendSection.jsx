...
function CompanySearchDropdown({ companyList, selectedCompany, onChange }) {
  ...
  return (
    <div ref={ref} className="relative">
      <div
        className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 dark:bg-white/8 border border-slate-200 dark:border-white/10 rounded-lg cursor-text min-w-[200px]"
      ...
      >
        {open ? (
          ...
          <span className="text-xs font-semibold text-slate-700 dark:text-slate-200 truncate flex-1" title={selectedCompany}>
            {selectedCompany || 'Şirket seç'}
          </span>
        )}
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map(c => (
                ...
                >
                  ...
                  <span className="text-slate-400 text-xs flex-shrink-0">₺{(c.netPremium / 1_000_000).toFixed(1)}M</span>
                </button>
              ))
            )}
          </div>
...
export default function CompanyTrendSection({ productGroup, defaultCompany, onCompanyChange, companyList = [], filter }) {
  ...
  const loadData = useCallback((company) => {
    if (!company) return
    setLoading(true)
    setError('')
    Promise.all([
      companyApi.getCompanyTrend(productGroup, company, filter),
      companyApi.getCompanyProfile(productGroup, company, filter),
    ])
  ...
  useEffect(() => {
    if (selectedCompany) loadData(selectedCompany)
  ...
  const aracBedeli     = (profileData?.profile ?? []).filter(x => x.category === 'AracBedeli').slice(0, 5)
  ...
  return (
    <div className="bg-white dark:bg-[#0d1f3c] border border-slate-200 dark:border-white/7 rounded-xl p-5 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">Şirket Trend & Profil</p>
        <CompanySearchDropdown companyList={companyList} selectedCompany={selectedCompany} onChange={handleCompanyChange} />
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 pt-2 border-t border-slate-100 dark:border-white/6">
        <div className="text-center">
          <p className="text-xs text-slate-400 dark:text-slate-500">Toplam Prim (8H)</p>
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-5">
          ...
          {aracBedeli.length > 0 && (
            <div className="col-span-2">
              <p className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wide mb-2">Araç Bedeli Dağılımı (Top 5)</p>
              <div className="flex flex-wrap gap-1.5">
                {aracBedeli.map(item => (
                  <div key={item.type} className="flex items-center gap-1 px-2 py-1 bg-slate-50 dark:bg-white/5 rounded-lg">
                    <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">{item.type}</span>
