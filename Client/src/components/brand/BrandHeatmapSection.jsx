...
export default function BrandHeatmapSection({ productGroup, filter }) {
  const [data,    setData]    = useState([])
  const [loading, setLoading] = useState(true)
  const [error,   setError]   = useState('')

  const fileName = useMemo(() => {
    if (!data.length) return `NetPrim_Marka_Heatmap_${productGroup}.xlsx`
    const first = data[0].week
    const last  = data[data.length - 1].week
    return `NetPrim_Marka_Heatmap_${productGroup}_${first}-${last}.xlsx`
  }, [data, productGroup])

  const { trigger: handleExport, loading: exportLoading } = useExport(
    () => exportBrandHeatmap(productGroup,filter),
    fileName
  )
  ...
  useEffect(() => {
    setLoading(true)
    setError('')
    brandApi.getBrandHeatmap(productGroup, filter)
      .then(r => setData(r.data))
      .catch(() => setError('Heatmap verileri yüklenemedi.'))
      .finally(() => setLoading(false))
  }, [productGroup, filter])

  if (loading) return <BrandHeatmapSkeleton />
  if (error)   return <div className="flex items-center justify-center py-10 text-sm text-red-400">{error}</div>

  const mapped = data.map(d => ({ brand: d.brand, week: d.week, avgNetPremium: d.avgNetPremium, policyRatio: d.policyRatio }))

  return (
    <div className="bg-white dark:bg-[#0d1f3c] border border-slate-200 dark:border-white/7 rounded-xl p-5">
      <HeatmapTable data={mapped} onExport={handleExport} exportLoading={exportLoading} />
    </div>
  )
}