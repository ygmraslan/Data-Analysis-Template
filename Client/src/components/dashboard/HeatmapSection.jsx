...
import { exportHeatmap } from '../../api/exportApi'
...
const HeatmapSkeleton = () => (
  <SkeletonCard>
    ...
    <div className="space-y-2">
      <SkeletonRow>
        <SkeletonBlock width="w-24" height="h-6" rounded="rounded-sm" />
...
export default function HeatmapSection({ productGroup, filter }) {
  const [data,    setData]    = useState([])
  const [loading, setLoading] = useState(true)
  const [error,   setError]   = useState('')

  const fileName = useMemo(() => {
    if (!data.length) return `NetPrim_Heatmap_${productGroup}.xlsx`
    const first = data[0].week
    const last  = data[data.length - 1].week
    return `NetPrim_Heatmap_${productGroup}_${first}-${last}.xlsx`
  }, [data, productGroup])

  const { trigger: handleExport, loading: exportLoading } = useExport(
    () => exportHeatmap(productGroup),
    fileName
  )
  ...
  useEffect(() => {
    setLoading(true)
    setError('')
    dashboardApi.getHeatmap(productGroup, filter)
  ...
  return (
    <div className="bg-white dark:bg-[#0d1f3c] border border-slate-200 dark:border-white/7 rounded-xl p-5">
      <HeatmapTable data={data} onExport={handleExport} exportLoading={exportLoading} />
    </div>
  )
}