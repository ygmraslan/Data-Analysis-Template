...
function getDateRange(shortcut) {
  ...
  if (shortcut === 'today') { start.setHours(0,0,0,0); return { startDate: start.toISOString(), endDate: now.toISOString() } }
  if (shortcut === 'week')  { start.setDate(now.getDate()-7); return { startDate: start.toISOString(), endDate: now.toISOString() } }
  if (shortcut === 'month') { start.setMonth(now.getMonth()-1); return { startDate: start.toISOString(), endDate: now.toISOString() } }
  return { startDate: '', endDate: '' }
}

function SuccessBadge({ success }) {
  return success
    ? <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold border bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/20">Başarılı</span>
    : <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold border bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-400 border-red-200 dark:border-red-500/20">Başarısız</span>
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-[#002147] rounded-2xl shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-white/8">
          <h3 className="text-sm font-bold text-slate-800 dark:text-white">Giriş Detayı</h3>
        ...
        <div className="p-5 space-y-3">
          {[
            { label: 'İsim',      value: log.fullName || '—' },
            { label: 'E-posta',   value: log.email || '—' },
            { label: 'Sonuç',     value: <SuccessBadge success={log.success} /> },
            { label: 'Sebep',     value: log.reason },
...
export default function AuthLogsList() {
  ...
  const buildParams = useCallback(() => {
    ...
    return params
  }, [page, dateShortcut, customStart, customEnd, successFilter, search])

  const fetchLogs = useCallback(async () => {
    setLoading(true)
    setError('')
    try {
      const response = await auditLogsApi.getAuthLogs(buildParams())
    ...
    <div className="bg-white dark:bg-[#002147] rounded-2xl border border-slate-200 dark:border-white/8 overflow-hidden">
      ...
      <div className="px-4 md:px-6 py-4 border-b border-slate-100 dark:border-white/8 space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <div className="flex items-center gap-1 bg-slate-100 dark:bg-white/8 rounded-lg p-1">
            {[{ label: 'Tümü', value: '' }, { label: 'Başarılı', value: 'true' }, { label: 'Başarısız', value: 'false' }].map((s) => (
              <button key={s.value} onClick={() => { setSuccessFilter(s.value); setPage(1) }}
                className={`flex-1 sm:flex-none whitespace-nowrap px-3 py-2 sm:py-1.5 rounded-md text-sm sm:text-xs font-semibold transition-all ${
                  successFilter === s.value
                    ? 'bg-white dark:bg-white/15 text-slate-800 dark:text-white shadow-sm'
                    : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'
                }`}>{s.label}
              </button>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-slate-100 dark:border-white/8 bg-slate-50/50 dark:bg-white/2">
                  {['İsim', 'E-posta', 'Sebep', 'Sonuç', 'IP / Tarayıcı', 'Tarih'].map((h) => (
                  ...
                  <tr key={log.id} onClick={() => setSelectedLog(log)}
                    ...
                    <td className="px-5 py-3.5">
                      {log.fullName
                        ...
                        : <p className="text-xs text-slate-400 dark:text-slate-500 italic">Bilinmiyor</p>
                      }
                    </td>
                    <td className="px-5 py-3.5 text-sm text-slate-700 dark:text-slate-300">{log.email || '—'}</td>
                    <td className="px-5 py-3.5 text-sm text-slate-600 dark:text-slate-400">{log.reason}</td>
                    <td className="px-5 py-3.5"><SuccessBadge success={log.success} /></td>
                    <td className="px-5 py-3.5">
          ...
          <div className="block md:hidden divide-y divide-slate-100 dark:divide-white/6">
            {logs.map((log) => (
              <div key={log.id} onClick={() => setSelectedLog(log)}
                className="p-4 space-y-2 cursor-pointer active:bg-slate-50 dark:active:bg-white/4 transition-colors">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    {log.fullName
                      ? <p className="text-sm font-semibold text-slate-800 dark:text-white">{log.fullName}</p>
                      : <p className="text-xs text-slate-400 italic">Bilinmiyor</p>
                    }
                    <p className="text-xs text-slate-400 mt-0.5">{log.email || '—'}</p>
                  </div>
                  <SuccessBadge success={log.success} />
                </div>
                <p className="text-xs text-slate-500 dark:text-slate-400">{log.reason}</p>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-slate-400">{log.browser} · {log.ipAddress}</span>
