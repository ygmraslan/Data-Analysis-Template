...
public class HeatmapExcelExport
{
    public byte[] Build(List<GetHeatmapQueryResponse> data, string productGroup, string filterSummary = "")
    {
    ...
    private void BuildPremiumSheet(XLWorkbook workbook, List<GetHeatmapQueryResponse> data, 
        List<string> brands, List<string> weeks, string productGroup, string dateRange)
    {
        ...
        ExcelBuilder.WriteTitle(ws, $"İlk 10 Marka — Ort. Yazılan Prim — {productGroup} — {dateRange}", weeks.Count + 1);
    ...
    private void BuildPolicyRatioSheet(XLWorkbook workbook, List<GetHeatmapQueryResponse> data,
        List<string> brands, List<string> weeks, string productGroup, string dateRange)
    {
        var ws = workbook.AddWorksheet("Poliçe Payı");
        var valueMap = data.ToDictionary(d => $"{d.Brand}__{d.Week}", d => d.PolicyRatio);

        ExcelBuilder.WriteTitle(ws, $"İlk 10 Marka — Poliçe Payı (%) — {productGroup} — {dateRange}", weeks.Count + 1);
        ...
        for (int b = 0; b < brands.Count; b++)
        {
            ...
            for (int w = 0; w < weeks.Count; w++)
            {
                ...
                if (valueMap.TryGetValue(key, out var val) && val > 0)
                {
                    cell.Value = (double)val / 100; // Yüzde formatı için
